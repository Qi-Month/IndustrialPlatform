...
public class FillAdjusterItem extends Item {
	...
	public static Target getTarget(ItemStack stack) {
		CompoundTag tag = stack.getTag();
		if (tag == null || !tag.contains(TARGET_KEY)) {
			return Target.UP;
		}
	...
	public static void setTarget(ItemStack stack, Target target) {
		stack.getOrCreateTag().putInt(TARGET_KEY, target.ordinal());
	}
	...
	/**
	 * 对方块右键: 站立时交给方块(平台方块/设计台照旧), 潜行时切换调节目标
	 */
	@Override
	...
	/**
	 * 切换目标并提示, 两端都从同一份 NBT 出发, 所以各自翻一次的结果是一致的
	 *
	 * @return 是否真的切了
	 */
