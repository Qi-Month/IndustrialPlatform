...
/**
 * 搭建界面旁边的平台预览: 按当前 样式/尺寸/上下填充 画出放置后的样子, 俯视角绕竖轴顺时针转
 */
public class PlatformPreviewWidget extends AbstractWidget implements AutoCloseable {
	...
	/**
	 * 旋转时占地的投影宽度最大是对角线, 高度再加上俯视角下压扁的填充高度, 按两者取能塞进视口的缩放
	 */
	...
	private void drawScene(PoseStack pose, float centerX, float centerY) {
		...
		// 界面的 Y 轴朝下, 和物品渲染一样翻转 Y 才能让模型正立且面剔除方向正确
		pose.mulPoseMatrix(flip.scaling(scale, -scale, scale));
	...
	/**
	 * 向上清理的范围: 和世界里的区块预览一样画成半透明蓝色的墙 + 线框
	 */
	private void drawClearBox(Matrix4f matrix) {
		...
		for (int corner = 0; corner < RING_CORNERS; corner++) {
			...
			builder.vertex(matrix, ringX[corner], minY, ringZ[corner]).color(red, green, blue, CLEAR_ALPHA).endVertex();
			builder.vertex(matrix, ringX[corner], maxY, ringZ[corner]).color(red, green, blue, CLEAR_ALPHA).endVertex();
			builder.vertex(matrix, ringX[next], maxY, ringZ[next]).color(red, green, blue, CLEAR_ALPHA).endVertex();
			builder.vertex(matrix, ringX[next], minY, ringZ[next]).color(red, green, blue, CLEAR_ALPHA).endVertex();
		}
		for (int corner = 0; corner < RING_CORNERS; corner++) {
			builder.vertex(matrix, ringX[corner], maxY, ringZ[corner]).color(red, green, blue, CLEAR_ALPHA).endVertex();
		}
		...
		for (int corner = 0; corner < RING_CORNERS; corner++) {
			int next = (corner + 1) % RING_CORNERS;
			builder.vertex(matrix, ringX[corner], minY, ringZ[corner]).color(red, green, blue, OUTLINE_ALPHA).endVertex();
			builder.vertex(matrix, ringX[next], minY, ringZ[next]).color(red, green, blue, OUTLINE_ALPHA).endVertex();
			builder.vertex(matrix, ringX[corner], maxY, ringZ[corner]).color(red, green, blue, OUTLINE_ALPHA).endVertex();
			builder.vertex(matrix, ringX[next], maxY, ringZ[next]).color(red, green, blue, OUTLINE_ALPHA).endVertex();
			builder.vertex(matrix, ringX[corner], minY, ringZ[corner]).color(red, green, blue, OUTLINE_ALPHA).endVertex();
			builder.vertex(matrix, ringX[corner], maxY, ringZ[corner]).color(red, green, blue, OUTLINE_ALPHA).endVertex();
		}
		tesselator.end();
